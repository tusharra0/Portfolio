...
const StaticName = () => {
  ...
  return (
    <h1
      className="text-5xl sm:text-6xl lg:text-7xl font-black tracking-tighter uppercase text-center select-none font-mono"
      style={{ cursor: 'none' }}
...
const App = () => {
  ...
  const work = [
    ...
    { logoSrc: TotalisLogo, company: "Totalis (YC P26)", role: "part time software engineer", desc: "derivative layer for prediction markets", date: "march 2026 - may 2026" },
  ...
  return (
    <div className="h-screen w-screen overflow-hidden flex flex-col items-center bg-[#050505] text-white" style={{ cursor: 'none' }}>
      <LineCursor />
      ...
      {/* Side accent bars */}
      <div className="fixed left-0 top-0 bottom-0 w-[1px] bg-gradient-to-b from-transparent via-[#e8630a]/20 to-transparent" />
      <div className="fixed right-0 top-0 bottom-0 w-[1px] bg-gradient-to-b from-transparent via-[#e8630a]/20 to-transparent" />
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto px-6 sm:px-10 lg:px-16 flex flex-col justify-center">
        <div className="grid grid-cols-1 md:grid-cols-[140px_1fr] gap-y-6 md:gap-x-12 lg:gap-x-20">
          ...
          <div className="space-y-0 mt-4 md:mt-6">
            {work.map((item, idx) => (
              <div key={idx} className="group flex items-center gap-3 sm:gap-4 py-2 sm:py-2.5 hover:bg-[#e8630a]/[0.03] rounded-lg px-2 -mx-2 transition-all duration-200 border border-transparent hover:border-[#e8630a]/10">
                <img src={item.logoSrc} alt={item.company} className="w-8 h-8 sm:w-9 sm:h-9 rounded-md object-cover flex-shrink-0 grayscale group-hover:grayscale-0 transition-all duration-500" />
